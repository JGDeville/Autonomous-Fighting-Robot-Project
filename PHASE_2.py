...
Metrics logged per run:
  ...
  - Path efficiency          (%)   straight-line displacement / actual path * 100
                                   (true 2D using robot pixel coordinates,
...
class PerformanceLogger:
    ...
    def end_run(self, label="run"):
        ...
        # True 2D path efficiency using actual robot pixel coordinates.
        # Identical calculation to Phase 1 so values are directly comparable.
...
def steer_to_motor_command(steering_error_deg, base_speed_us=None):
    ...
    Phase 1 used hard LEFT/RIGHT spin (full stop, then spin, then drive).
    Phase 2 maps error continuously:
      0deg error   -> both wheels at PWM_FWD, straight forward
      45deg error  -> one wheel at PWM_STOP, gentle arc while moving
      90deg+ error -> one wheel at PWM_REV, tight spin while still moving

    The robot never fully stops to align.
    """
    if base_speed_us is None:
        base_speed_us = PWM_FWD
    t         = np.clip(steering_error_deg / MAX_STEER_DEG, -1.0, 1.0)
...
def decide_state_and_command(detections, last_valid_cmd, last_detection_time):
    ...
    Both markers visible:
      distance <= RAM_THRESHOLD  -> RAMMING,  FORWARD
      otherwise                  -> ATTACKING, proportional MOTOR command

    Detection dropped:
      within HOLD_DURATION       -> HOLDING,   last valid command
      beyond HOLD_DURATION       -> SEARCHING, STOP
    """
    ...
    if ROBOT_ID in detections and OPPONENT_ID in detections:
        ...
        if dst <= RAM_THRESHOLD_PX:
            return State.RAMMING, "FORWARD", info, "FORWARD", now

        cmd = steer_to_motor_command(err)
        return State.ATTACKING, cmd, info, cmd, now

    else:
        time_since = now - last_detection_time
        if last_valid_cmd is not None and time_since < HOLD_DURATION:
            return State.HOLDING, last_valid_cmd, info, last_valid_cmd, last_detection_time
        else:
...
def main():
    ...
    print("PHASE 2 -- Proportional Differential Drive")
    ...
    try:
        while cap.isOpened():
            ...
            if autonomous:
                current_state, current_cmd, info, last_valid_cmd, last_detection_time = \
                    decide_state_and_command(detections, last_valid_cmd, last_detection_time)
            else:
                current_state       = State.SEARCHING
                current_cmd         = "STOP"
                info                = {'steering_error': None, 'distance': None}
            ...
            if (logger.running
                ...
                logger.end_run(label=f"phase2_run{run_counter:02d}")

            frame = draw_hud(frame, detections, current_state,
                             current_cmd, info, worker, autonomous, logger)
            cv2.imshow("Robot Vision -- Phase 2", frame)

            key = cv2.waitKey(1) & 0xFF

            if key == ord('q'):
                if logger.running:
                    logger.end_run(label=f"phase2_run{run_counter:02d}_aborted")
                print("\n[SYS] Quitting.")
                break
                ...
                last_valid_cmd = None
                if logger.running:
                    logger.end_run(label=f"phase2_run{run_counter:02d}_stopped")
                with worker.queue.mutex:
                    worker.queue.queue.clear()
                worker.put("STOP")
                last_sent_cmd = "STOP"
        ...
        print("\n[SYS] Interrupted.")
        if logger.running:
            logger.end_run(label=f"phase2_run{run_counter:02d}_interrupted")
